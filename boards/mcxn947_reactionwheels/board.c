...
#if DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(flexpwm1_pwm3))
    /* 1. Enable the Main FlexPWM1 Bus Clock */
    /* Found in SDK as: kCLOCK_Pwm1 */
    CLOCK_EnableClock(kCLOCK_Pwm1);

    /* 2. Enable the Specific Submodule Clocks */
    /* The grep showed these exist, so we must turn them on explicitly! */
