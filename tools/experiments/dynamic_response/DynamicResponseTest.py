...
class ZephyrThread(threading.Thread):
    ...
    def run(self):
        # Wait for LabJack tare before triggering firmware so the torque zero
        # reference corresponds to the pre-test condition.
...
def write_csv(path, shared, output_rate_hz, phase_resistance_ohm):
    ...
    with open(path, "w", newline="") as f:
        ...
        writer.writerow([
            ...
            "step_command_delta_rpm",
            "abs_speed_error_rpm",
        ])

        previous_command_rpm = math.nan
        ...
        for i in range(n):
            ...
            if math.isfinite(speed_cmd_rpm) and math.isfinite(previous_command_rpm):
                step_command_delta_rpm = speed_cmd_rpm - previous_command_rpm
            else:
                step_command_delta_rpm = math.nan

            previous_command_rpm = speed_cmd_rpm

            abs_speed_error_rpm = (
                abs(speed_cmd_rpm - omega_rpm)
                if math.isfinite(speed_cmd_rpm) and math.isfinite(omega_rpm)
                else math.nan
            )
...
def main():
    parser = argparse.ArgumentParser(description="DynamicResponseTest DAQ capture")

    parser.add_argument("--baud", type=int, default=DEFAULT_BAUD)
    parser.add_argument("--out", default="dynamic_response_raw.csv")
