...
class ZephyrThread(threading.Thread):
    ...
    def run(self):
        # Wait for LabJack tare before triggering firmware so the torque
        # reference corresponds to the pre-test zero condition.
        ...
        try:
            ...
            while not self.shared.stop_event.is_set():
                ...
                for i, voltage in enumerate(data):
                    ...
                    torque_delta_v = float(voltage) - self.tare_voltage
                    torque_nm = torque_delta_v / TORQUE_SENSOR_V_PER_NM

                    rows.append([
                        host_time_s,
                        float(voltage),
                        float(torque_delta_v),
                        float(torque_nm),
                    ])
...
def write_csv(path, shared, output_rate_hz, phase_resistance_ohm):
    ...
    with open(path, "w", newline="") as f:
        ...
        for i in range(n):
            ...
            eta_global = (
                p_mech / p_dc_in
                if math.isfinite(p_mech) and math.isfinite(p_dc_in) and abs(p_dc_in) > 1e-9
                else math.nan
            )
...
def main():
    parser = argparse.ArgumentParser(description="PowerRippleTest DAQ capture")

    parser.add_argument("--baud", type=int, default=DEFAULT_BAUD)
    parser.add_argument("--out", default="power_ripple_raw.csv")

    parser.add_argument("--output-rate-hz", type=float, default=200.0)
    parser.add_argument("--max-duration-s", type=float, default=800.0)
